...
def make_zoo_vec_env(env_name, victim, victim_type, victim_index, no_normalize,
                     seed, out_dir, vector):
    def agent_fn(env, sess):
        return get_agent_any_type(agent=victim, agent_type=victim_type, env=env, env_name=env_name,
...
def ppo_baseline(_run, env, victim, victim_type, out_dir, exp_name, vectorize,
    ...
    env = make_zoo_vec_env(env_name=env, victim=victim, victim_type=victim_type, victim_index=0,
                           no_normalize=no_normalize, seed=seed, out_dir=out_dir, vector=vectorize)
