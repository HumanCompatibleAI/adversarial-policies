...
def lineplot_multi_fig(outer_key, data, xcol, ycols, ci, in_split_keys,
                       plot_cfg=None, data_fns=None, plot_fns=None, **kwargs):
...
def lineplot_monolithic(outer_key, data, xcol, ycols, ci, in_split_keys,
                        plot_cfg, data_fns=None, plot_fns=None, **kwargs):
    """General method for line plotting TensorBoard datasets with smoothing and subsampling.
...
def win_rate(tb_dir, lineplot_fn, out_split_keys, in_split_keys, data_fns, plot_fns, fig_dir,
             xcol, ci, plot_cfg, ycols=None, **kwargs):
    if ycols is None:
        ycols = ['Opponent Win', 'Victim Win', 'Ties']

    configs, events = tb.load_tb_data(tb_dir, keys=['game_win0', 'game_win1', 'game_tie'])
    make_fig_wrapped = functools.partial(_win_rate_make_fig, lineplot_fn=lineplot_fn,
                                         xcol=xcol, ycols=ycols, ci=ci,
                                         plot_cfg=plot_cfg, in_split_keys=in_split_keys,
                                         data_fns=[_win_rate_data_convert] + data_fns,
                                         plot_fns=[_win_rate_labels] + plot_fns,
                                         fig_dir=fig_dir, **kwargs)
...
def win_rate_per_victim_env(tb_dir, baseline):
    ...
    return win_rate(tb_dir, lineplot_multi_fig, out_split_keys, in_split_keys,
...
def win_rate_per_env(tb_dir, baseline):
    ...
    return win_rate(tb_dir, lineplot_multi_fig, out_split_keys, in_split_keys,
                    data_fns=[], plot_fns=[], hue='victim_path')
...
def _gen_cell(env_name, victim_path):
    return {
        ...
        'title': f'{util.PRETTY_ENV.get(env_name, env_name)} {victim_path}',
    }
