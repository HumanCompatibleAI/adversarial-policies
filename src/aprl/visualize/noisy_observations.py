...
from aprl.envs.gym_compete import NUM_ZOO_POLICIES
...
def generate_plots(input_run, root_dir, out_dir):
    ...
    baseline_transformations = [
        {"new_col": "log_noise", "old_col": "noise_magnitude", "func": lambda x: math.log(x)},
        {
            "new_col": "agent0_win_perc",
            "old_col": "agent0_wins",
            "func": lambda x: x / num_episodes,
        },
        {
            "new_col": "agent1_win_perc",
            "old_col": "agent1_wins",
            "func": lambda x: x / num_episodes,
        },
    ]
    ...
    for env_name, pretty_env in styles.PRETTY_ENV.items():
        ...
        for zoo_id in range(1, NUM_ZOO_POLICIES[short_env] + 1):
            subset_params = {"agent0_path": str(zoo_id), "env": env_name}

            zoo_plot_path = os.path.join(
                experiment_out_dir, f"{input_run}_ZooBaseline_" f"{short_env}_AgainstZoo{zoo_id}"
            )

            adversary_plot_path = os.path.join(
                experiment_out_dir,
                f"{input_run}_AdversaryTrained_" f"{short_env}_AgainstZoo{zoo_id}",
            )
            noisy_multiple_opponent_subset_plot(
                noisy_zoo_obs_df,
                subset_specs=subset_params,
                transform_specs=baseline_transformations,
                savefile=zoo_plot_path,
            )
            noisy_adversary_opponent_subset_plot(
                noisy_adv_obs_df,
                subset_specs=subset_params,
                transform_specs=baseline_transformations,
