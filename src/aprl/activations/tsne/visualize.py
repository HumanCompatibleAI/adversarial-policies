...
logger = logging.getLogger("aprl.activations.tsne.visualize")
...
def main_config():
    ...
    pretty_labels = vis_styles.PRETTY_LABELS
...
PALETTES = {
    "paper": {"Adv": "#1f77b4", "Zoo": "#ff7f0e", "Rand": "#2ca02c"},
    "bright": {"Adv": "#e7298a", "Zoo": "#66c2a5", "Rand": "#fdb462"},
    "cube_bright": {"Adv": "#016b61", "Zoo": "#b87903", "Rand": "#d2b5ff"},
}
...
def _make_handles(palette_name, ordering, pretty_labels):
    ...
    for key in ordering:
        ...
        handle = matplotlib.lines.Line2D(
            range(1), range(1), color=color, marker="o", markerfacecolor=color, linewidth=0
        )
...
def _plot_and_save_chart(
    ...
    with plt.style.context([vis_styles.STYLES[style] for style in styles]):
        ...
        for data, ax in zip(datasets, axs[0]):
            ...
            if internal_legend is not None:
                ...
                fig.legend(
                    ...
                    ncol=len(handles),
                    loc="lower center",
                    bbox_to_anchor=(0.05, 0.88, 0.9, 0.05),
                    mode="expand",
                    borderaxespad=0,
                    frameon=True,
                )
